...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  #drawer {{ position: absolute; top: 0; right: 0; height: 100%; width: 310px; background: rgba(15,34,30,0.94);
...
function renderDrawer() {{
  ...
  if (selected.type === 'household') {{
    ...
    el.innerHTML = `
      ...
      <div class="note">${{hh.last_reasoning || "Hasn't reasoned yet — step a round from the sidebar."}}</div>
    `;
    ...
    el.innerHTML = `
      ...
      <div class="note">${{firm.last_reasoning || "Hasn't reasoned yet — step a round from the sidebar."}}</div>
    `;
  }} else {{
